...
public class DbInstitution extends InstanceManager implements IController {
	private String select = "select * from bayer_instution ";
	private String insert = "insert into bayer_instution ";
	private String update = "update bayer_instution set ";
	public InstutionModel row;
	
	public DbInstitution(ConnectionManager connectionManager) {
		this.setConnectionManager(connectionManager);
		this.row = new InstutionModel();
	}	
	...
	public boolean post() {
		...
		try {
			...
			if (this.getDbState() == "insert") {
				sql = this.insert.concat(" (socialreason, cnpj, AT_CREATE, AT_UPDATE)");
				sql = sql.concat(" values (?,?,?,?)");
				
				pstmt = this.connectionManager.getInstance().prepareStatement(sql);
				pstmt.setString(1, this.row.getSocialReason());
				pstmt.setString(2, this.row.getCNPJ());				
				pstmt.setDate(3, this.row.getAtUpdateSQLDate());				
				pstmt.setDate(4, this.row.getAtCreateSQLDate());
			} else if (this.getDbState() == "edit") {								
				sql = this.update.concat(" socialreason = ?, cnpj = ?, AT_UPDATE = ?");
